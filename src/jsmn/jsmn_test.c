...
static char* canopy_test1 = "{ \"result\" : \"ok\","
...
int test_canopy() {
	...
	js = canopy2;
	memset(&tokens, 0, sizeof(jsmntok_t) * 128);
	jsmn_init(&p);
	r = jsmn_parse(&p, js, strlen(js), tokens, 128);
	check(r >= 0);
	printf("input:\n\t%s\n", js);
	dump(js, &tokens[0], r, 0);
#if 0
	for (i = 1; tokens[i].end < tokens[0].end; i++) {
		if (tokens[ti].type == JSMN_STRING || tokens[ti].type == JSMN_PRIMITIVE) {
			printf("\t%.*s   %s", tokens[ti].end - tokens[ti].start, js + tokens[ti].start, (tokens[ti].size == 0 ? "\n" : ""));
		} else if (tokens[ti].type == JSMN_ARRAY) {
			printf("[\t%d elems]\n", tokens[ti].size);
		} else if (tokens[ti].type == JSMN_OBJECT) {
			printf("\t{%d elems}\n", tokens[ti].size);
		} else {
			TOKEN_PRINT(tokens[ti]);
		}
		ti++;
		if (ti > r) {
			break;
...
int main() {
	...
	test(test_canopy, "test for canopy");
